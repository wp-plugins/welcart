...
class usc_e_shop
{
	...
	function usc_e_shop()
	{
		...
		if(!isset($this->options['devide_item'])) $this->options['devide_item'] = 0;
	...
	function add_pages() {
		...
		add_submenu_page(USCES_PLUGIN_BASENAME, 'ホーム', 'ホーム', 6, USCES_PLUGIN_BASENAME, array($this, 'admin_top_page'));
//		add_submenu_page(USCES_PLUGIN_BASENAME, '商品マスター', '<img src="'.get_option('siteurl').'/wp-content/plugins/usc-e-shop/images/easymoblog2.png" /> 商品マスター', 6, 'usces_itemedit', array($this, 'item_master_page'));
		add_submenu_page(USCES_PLUGIN_BASENAME, '商品マスター', '商品マスター', 6, 'usces_itemedit', array($this, 'item_master_page'));
		add_submenu_page(USCES_PLUGIN_BASENAME, '新規商品追加', '新規商品追加', 6, 'usces_itemnew', array($this, 'item_master_page'));
		add_submenu_page(USCES_PLUGIN_BASENAME, '基本設定', '基本設定', 6, 'usces_initial', array($this, 'admin_setup_page'));
		add_submenu_page(USCES_PLUGIN_BASENAME, '営業日設定', '営業日設定', 6, 'usces_schedule', array($this, 'admin_schedule_page'));
		add_submenu_page(USCES_PLUGIN_BASENAME, '配送設定', '配送設定', 6, 'usces_delivery', array($this, 'admin_delivery_page'));
		add_submenu_page(USCES_PLUGIN_BASENAME, 'メール設定', 'メール設定', 6, 'usces_mail', array($this, 'admin_mail_page'));
		add_submenu_page(USCES_PLUGIN_BASENAME, 'システム設定', 'システム設定', 6, 'usces_system', array($this, 'admin_system_page'));
		//add_submenu_page(USCES_PLUGIN_BASENAME, 'バックアップ', 'バックアップ', 6, 'usces_backup', array($this, 'admin_backup_page'));
		
		add_object_page('Welcart Management', 'Welcart Management', 6, 'usces_orderlist', array($this, 'order_list_page'));
		add_submenu_page('usces_orderlist', '受注リスト', '受注リスト', 6, 'usces_orderlist', array($this, 'order_list_page'));
		add_submenu_page('usces_orderlist', '新規受注見積登録', '新規受注見積登録', 6, 'usces_ordernew', array($this, 'order_list_page'));
		add_submenu_page('usces_orderlist', '会員リスト', '会員リスト', 6, 'usces_memberlist', array($this, 'member_list_page'));
	}
	...
	function order_list_page() {
		...
		switch ($order_action) {
			...
			case 'editpost':
				...
				if ( 1 === $res ) {
					$this->set_action_status('success', '受注データを更新しました。　　<a href="'.stripslashes( $_POST['usces_referer'] ).'">一覧に戻る</a>');
				} elseif ( 0 === $res ) {
					$this->set_action_status('none', '');
				} else {
					$this->set_action_status('error', 'ERROR：更新できませんでした。');
				}
			...
			case 'newpost':
				...
				if ( 1 === $res ) {
					$this->set_action_status('success', '新規データを追加しました。');
				} elseif ( 0 === $res ) {
					$this->set_action_status('none', '');
				} else {
					$this->set_action_status('error', 'ERROR：追加できませんでした。');
				}
			...
			case 'delete':
				...
				if ( 1 === $res ) {
					$this->set_action_status('success', '受注データを削除しました。');
				} elseif ( 0 === $res ) {
					$this->set_action_status('none', '');
				} else {
					$this->set_action_status('error', 'ERROR：削除できませんでした。');
				}
			default:
		...
		switch ($member_action) {
			case 'editpost':
				...
				if($this->error_message == ''){
					...
					if ( 1 === $res ) {
						$this->set_action_status('success', '会員情報を更新しました。');
					} elseif ( 0 === $res ) {
						$this->set_action_status('none', '');
					} else {
						$this->set_action_status('error', 'ERROR：更新できませんでした。');
			...
			case 'delete':
				...
				if ( 1 === $res ) {
					$this->set_action_status('success', '会員データを削除しました。');
				} elseif ( 0 === $res ) {
					$this->set_action_status('none', '');
				} else {
					$this->set_action_status('error', 'ERROR：削除できませんでした。');
	...
	function admin_setup_page() {
		...
		if(isset($_POST['usces_option_update'])) {
			...
			$this->action_message = 'オプションを更新しました';
		} else {
			$this->action_status = 'none';
			$this->action_message = '';
		}
	...
	function admin_system_page() {
		...
		if(isset($_POST['usces_option_update'])) {
			...
			$this->options['devide_item'] = isset($_POST['devide_item']) ? 1 : 0;
...
//		$wpdb->print_error();
	...
	function filter_cartContent($content) {
		...
		switch($this->page){
			...
			case 'ordercompletion':
				...
				break;
			case 'orderdownload':
				if(function_exists('dlseller_download'))
					dlseller_dlpage();
				break;
			case 'error':
	...
	function filter_devide_item(){
		global $wp_query;

		if( $usces->options['devide_item'] && !is_category() && !is_search() && !is_singular() && !is_admin() ){
